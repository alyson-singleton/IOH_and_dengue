...
```{r, STable5}
stable5 <- etable_glmmTMB[,c(2:7)]
colnames(stable5) <- c('Dengue Yearly', 'Dengue Biannual Dry', 'Dengue Biannual Rainy','Leish Yearly','Leish Biannual Dry', 'Leish Biannual Rainy')
stable5 <- as.data.frame(rbind(stable5, stable3[c(7:10),]))
stable5 <- stable5 %>%
  mutate(across(everything(), ~ ifelse(is.na(.), "", .)))
rownames(stable5) <- c("5km x Post-2008 Yearly", "5km x Post-2008 Biannual", "Urban", "Ag", "Sum Precip", "Mean Temp^2", "FEs", "Unit", "Year", "Biannual")
stable5
...
# --- Final clean table ---
stable6 <- dengue_yearly_df_full %>%
  dplyr::select(Term = term_clean, `Estimate (SE)` = formatted)
stable6 <- final_table[c(1,2,4,3,7,8,5,6,9:101),]
stable6[2:8,1] <- c("5km x Post-2008 Yearly", "5km", "Post-2008 Yearly", "Urban", "Ag", "Sum Precip", "Mean Temp^2")
stable6
...
sfig3 <- ggplot(sfig3_df, aes(group=Model)) +
  ...
  scale_color_manual(name = "Model specification", values=list('No PM'='darkred', 'No LUC' = '#DC267F', 
                                                               ...
                                                               'Main' = "black"),
                     labels=list('No PM', 'No LUC', 'Pop weighting','1km boundary','10km boundary',
