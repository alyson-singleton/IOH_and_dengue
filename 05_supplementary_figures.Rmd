...
###################
### yearly data
dengue_df_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/dengue_yearly_full_dataset.csv")
...
###################
### yearly data
leish_df_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/leish_yearly_full_dataset.csv")
...
#############################
### yearly data
dengue_df_yearly_confirmed <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/dengue_yearly_full_dataset_confirmed.csv")
...
###############################################################################
### yearly data
dengue_df_yearly_prepost <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km_prepost_req/dengue_yearly_full_dataset.csv")
...
###################
### biannual data
dengue_df_biannual <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/dengue_biannual_full_dataset.csv")
...
### biannaul data
leish_df_biannual <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/leish_biannual_full_dataset.csv")
...
dengue_yearly_model_confirmed_agg <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_agg_buffered_confirmed)
...
stable2[10,9] <- 49
colnames(stable2) <- c('Main model', 'No PM', 'Pop weight no PM','1km boundary','10km boundary','No buffer (<5km v >5km)', 'Large buffer (<5km v >20km)', 'Confirmed cases', 'Units reporting pre & post')
...
## confirmed cases only v confirmed and probable cases
dengue_yearly_model_confirmed <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_confirmed_buffered)
...
sfig3a <- ggplot(df, aes(group=Model)) +
  ...
  scale_color_manual(name = "Model specification", values=list('No PM'='#DC267F','Pop weight no PM'="#FE6100",'1km'='#FFB000','10km'='#FFE518','No buffer'='#85CE76', 'Big buffer'='#126E29','Confirmed cases'="#648FFF", 'Pre post'="#785EF0", 'Main' = "#2b175e"),
                     labels=list('No PM', 'Pop weight no PM','1km boundary','10km boundary','No buffer (<5km v >5km)', 'Large buffer (<5km v >20km)', 'Confirmed cases', 'Units reporting pre & post', 'Main model'))+
...
```{r, clustering sup figure}
#####################
## Load alternate clustering data and process
#####################
### 6km // 78 units
dengue_df_yearly_6km <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_6km/dengue_yearly_full_dataset.csv")
...
### 5km // 81 units
dengue_df_yearly_5km <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_5km/dengue_yearly_full_dataset.csv")
...
df$upper <- exp(df$upper)-1
df$lower <- exp(df$lower)-1
df$Model <- factor(as.character(df$Model), levels=c('7.5km clustering', '6km clustering','5km clustering'))

sfig10a <- ggplot(df, aes(group=Model)) +
  geom_hline(aes(yintercept=0), colour='red', linewidth=.4) +
  geom_errorbar(aes(x=year, ymax=upper, ymin=lower, color=Model), width=0.1, linewidth=0.5, alpha=0.8) +
  geom_vline(aes(xintercept=as.Date("2008-01-01")), linetype='dashed', linewidth=0.4) +
  geom_point(aes(x=as.Date("2008-01-01"), y=0), size=3, shape=21, fill='white') +
  geom_point(aes(year, estimate, fill=Model), color='black', size=3, shape=21, alpha=0.7) +
  scale_color_manual(name = "Clustering specification", values=list('7.5km clustering'='#DC267F','6km clustering'='#FFD218','5km clustering'="#648FFF"),
                     labels=list('7.5km clustering (n = 71)', '6km clustering (n = 78)','5km clustering (n = 84)'))+
  scale_fill_manual(name = "Clustering specification", values=list('7.5km clustering'='#DC267F','6km clustering'='#FFD218','5km clustering'="#648FFF"),
                     labels=list('7.5km clustering (n = 71)', '6km clustering (n = 78)','5km clustering (n = 84)'))+
  xlab("Year") + ylab("Percent\nchange\ndengue\nincidence") + 
  theme_bw()+
  scale_y_continuous(labels = scales::percent) +
...
sfig10b
sfig10_legend <- get_legend(sfig10b)
sfig10b <- sfig10b + theme(legend.position = "none")
...
## SFig 10all
#####################
sfig10all <- grid.arrange(sfig10b, sfig10c, sfig10d, sfig10a,                     
                         ncol = 4, nrow = 2,
                         layout_matrix = rbind(c(1,2,3),c(1,2,3),c(5,5,5),c(5,5,5)))

sfig10all <- as_ggplot(sfig10all) +                                
  draw_plot_label(label = c("A", "B", "C"), size = 17,
                  x = c(0.01, 0.33,0.66), y = c(0.95, 0.95, 0.95)) 

sfig10all

ggsave("SFig10.pdf", plot=sfig10all, "~/Desktop/doctorate/ch2 mdd highway/supplementary_figures/", width = 9, height = 5, units = c("in"), device="pdf")
```
