...
#for harper
# dengue_df_monthly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/dengue_monthly_full_dataset_c.csv")
# 
# dengue_df_monthly <- dengue_df_monthly %>%
#   group_by(tenkm,month) %>%
#   summarise(monthly_cases = sum(monthly_cases),
#             population = sum(population)) %>%
#   rename(group = tenkm)
# 
# write.csv(dengue_df_monthly,"~/Desktop/doctorate/ch2 mdd highway/data/harper_case_counts.csv")

###################
### dengue data ###
###################
### yearly data
dengue_df_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/dengue_yearly_full_dataset_c.csv")
...
###################
### yearly data
leish_df_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/leish_yearly_full_dataset_c.csv")
...
##########################################
### dengue data confirmed and probable ###
##########################################
### yearly data
dengue_df_yearly_cp <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/dengue_yearly_full_dataset_cp.csv")
dengue_df_yearly_cp$year <- as.factor(dengue_df_yearly_cp$year)

# cases + 1 so 0's dont go to infinity when logged
dengue_df_yearly_cp$incidence <- (dengue_df_yearly_cp$yearly_cases+1)/dengue_df_yearly_cp$population 

# land use vars + 0.001 so 0's dont go to infinity when logged
dengue_df_yearly_cp$urban <- dengue_df_yearly_cp$urban + 0.001
dengue_df_yearly_cp$ag <- dengue_df_yearly_cp$ag + 0.001

# double check complete cases
dengue_df_yearly_cp <- dengue_df_yearly_cp[complete.cases(dengue_df_yearly_cp),]

# remove extreme outliers (all most likely due to inaccurate pop data)
dengue_df_yearly_cp <- dengue_df_yearly_cp[which(dengue_df_yearly_cp$incidence < 0.5),]

# check how many are always zero
dengue_df_yearly_zeroes <- dengue_df_yearly_cp %>% 
  group_by(cluster) %>% 
  summarize(total_cases = sum(yearly_cases))
clusters_w_at_least_one_case <- dengue_df_yearly_cp$cluster[which(dengue_df_yearly_zeroes$total_cases>0)]
dengue_df_yearly_at_least_one_case <- dengue_df_yearly_cp[which(dengue_df_yearly_cp$cluster %in% clusters_w_at_least_one_case),]

# remove PM
dengue_df_yearly_cp_no_pm <- dengue_df_yearly_cp[!(dengue_df_yearly_cp$cluster %in% 1),]

# build final, buffered dataset where below 5km is 1, between 5km and 10km are thrown out (buffer zone), and above 10km is 0
dengue_df_yearly_cp_buffered <- dengue_df_yearly_cp[which(dengue_df_yearly_cp$all_cutoffs %in% c(1,2,4,5,6,7,0)),]
dengue_df_yearly_cp_buffered_no_pm <- dengue_df_yearly_cp_buffered[!(dengue_df_yearly_cp_buffered$cluster %in% 1),]

###############################################################################
### dengue data only clusters w at least one disease case reported pre/post ###
###############################################################################
### yearly data
dengue_df_yearly_prepost <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km_prepost_req/dengue_yearly_full_dataset_c.csv")
dengue_df_yearly_prepost$year <- as.factor(dengue_df_yearly_prepost$year)

# cases + 1 so 0's dont go to infinity when logged
dengue_df_yearly_prepost$incidence <- (dengue_df_yearly_prepost$yearly_cases+1)/dengue_df_yearly_prepost$population 

# land use vars + 0.001 so 0's dont go to infinity when logged
dengue_df_yearly_prepost$urban <- dengue_df_yearly_prepost$urban + 0.001
dengue_df_yearly_prepost$ag <- dengue_df_yearly_prepost$ag + 0.001

# double check complete cases
dengue_df_yearly_prepost <- dengue_df_yearly_prepost[complete.cases(dengue_df_yearly_prepost),]

# remove extreme outliers (all most likely due to inaccurate pop data)
dengue_df_yearly_prepost <- dengue_df_yearly_prepost[which(dengue_df_yearly_prepost$incidence < 0.5),]

# check how many are always zero
dengue_df_yearly_zeroes <- dengue_df_yearly_prepost %>% 
  group_by(cluster) %>% 
  summarize(total_cases = sum(yearly_cases))
clusters_w_at_least_one_case <- dengue_df_yearly_prepost$cluster[which(dengue_df_yearly_zeroes$total_cases>0)]
dengue_df_yearly_at_least_one_case <- dengue_df_yearly_prepost[which(dengue_df_yearly_prepost$cluster %in% clusters_w_at_least_one_case),]

# remove PM
dengue_df_yearly_prepost_no_pm <- dengue_df_yearly_prepost[!(dengue_df_yearly_prepost$cluster %in% 1),]

# build final, buffered dataset where below 5km is 1, between 5km and 10km are thrown out (buffer zone), and above 10km is 0
dengue_df_yearly_prepost_buffered <- dengue_df_yearly_prepost[which(dengue_df_yearly_prepost$all_cutoffs %in% c(1,2,4,5,6,7,0)),]
dengue_df_yearly_prepost_buffered_no_pm <- dengue_df_yearly_prepost_buffered[!(dengue_df_yearly_prepost_buffered$cluster %in% 1),]
```
...
###################
### biannual data
dengue_df_biannual <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/dengue_biannual_full_dataset_c.csv")
...
### biannaul data
leish_df_biannual <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data_7.5km/leish_biannual_full_dataset_c.csv")
...
```{r, STable1}
dengue_yearly_model <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_buffered)
leish_yearly_model <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + sum_precip + mean_temp + log(urban) + log(ag) | cluster + year, vcov = "cluster", data = leish_df_yearly_buffered)
...
rownames(stable1) <- c("5km x 2000", "5km x 2001", "5km x 2002", "5km x 2003", "5km x 2004", "5km x 2005", "5km x 2006", "5km x 2007", "5km x 2008", "5km x 2009", "5km x 2010", "5km x 2011", "5km x 2012", "5km x 2013", "5km x 2014", "5km x 2015", "5km x 2016", "5km x 2017", "5km x 2018", "5km x 2019", "5km x 2020", "5km x 2021", "5km x 2022", "log(Urban)", "log(Ag)", "Sum Precip", "Mean Temp", "FEs", "Cluster", "Year", "Cor^2", "N", "Clusters")
stable1
...
dengue_df_agg_buffered_cp <- dengue_df_yearly_cp_buffered
dengue_df_agg_buffered_cp <- dengue_df_agg_buffered_cp[which(as.Date(dengue_df_agg_buffered_cp$year) > '2007-01-01'),]
dengue_df_agg_buffered_cp$year_binary <- ifelse(as.Date(dengue_df_agg_buffered_cp$year) > '2008-01-01',1,0)

dengue_df_agg_buffered_prepost <- dengue_df_yearly_prepost_buffered
dengue_df_agg_buffered_prepost <- dengue_df_agg_buffered_prepost[which(as.Date(dengue_df_agg_buffered_prepost$year) > '2007-01-01'),]
dengue_df_agg_buffered_prepost$year_binary <- ifelse(as.Date(dengue_df_agg_buffered_prepost$year) > '2008-01-01',1,0)

## main specification
dengue_yearly_model_main_agg <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_agg_buffered)

## population weighting
dengue_df_agg_buffered_no_pm <- dengue_df_agg_buffered[!(dengue_df_agg_buffered$cluster %in% 1),]
dengue_yearly_model_pop_weight_agg <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_agg_buffered_no_pm, weights=~population)

## no Puerto Maldonado
dengue_yearly_model_no_PM_agg <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_agg_buffered_no_pm)

## changing the spatial boundary delineating treatment and control groups
dengue_yearly_model_onekm_agg <- fepois(incidence ~ year_binary*onekm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_agg)
dengue_yearly_model_tenkm_agg <- fepois(incidence ~ year_binary*tenkm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_agg)

## changing the buffer zone size between treatment and control groups
dengue_yearly_model_no_buffer_agg <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_agg)
dengue_df_yearly_buffer_bigger_agg <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(1,2,5,6,7,0)),]
dengue_yearly_model_buffer_bigger_agg <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_buffer_bigger_agg)

## confirmed cases only v confirmed and probable cases
dengue_yearly_model_confirmed_agg <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_agg_buffered_cp)

## only include clusters where at least one disease case reported both pre and post treatment
dengue_yearly_model_prepost_agg <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_agg_buffered_prepost)
...
stable2 <- etable(dengue_yearly_model_main_agg,dengue_yearly_model_no_PM_agg,dengue_yearly_model_pop_weight_agg,dengue_yearly_model_onekm_agg,
                  dengue_yearly_model_tenkm_agg,dengue_yearly_model_no_buffer_agg,dengue_yearly_model_buffer_bigger_agg,dengue_yearly_model_confirmed_agg, dengue_yearly_model_prepost_agg, digits = 2)
stable2 <- stable2[c(3:9,16,15,14),2:10]
stable2[10,] <- (as.numeric(gsub(",","",stable2[9,]))+1)/15
stable2[10,9] <- 49
colnames(stable2) <- c('Main model', 'No PM', 'Pop weight no PM','1km boundary','10km boundary','No buffer (<5km v >5km)', 'Large buffer (<5km v >20km)', 'Conf & prob cases', 'Units reporting pre & post')
rownames(stable2) <- c("log(Urban)", "log(Ag)", "Sum Precip", "Mean Temp", "5km x Post-2008", "1km x Post-2008", "10km x Post-2008", "Cor^2", "N", "Clusters")
...
## Build aggregated models
#####################
dengue_yearly_model_agg_quad <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip^2 + mean_temp^2 | cluster + year, 
                                  vcov = "cluster", 
                                  data = dengue_df_agg)
dengue_biannual_agg_model_dry_quad <- fepois(incidence ~ biannual_binary*fivekm + log(urban) + log(ag) + sum_precip^2 + mean_temp^2 | cluster + biannual_date, vcov = "cluster", data = dengue_df_agg_biannual_dry)
dengue_biannual_agg_model_rainy_quad <- fepois(incidence ~ biannual_binary*fivekm + log(urban) + log(ag) + sum_precip^2 + mean_temp^2 | cluster + biannual_date, vcov = "cluster", data = dengue_df_agg_biannual_rainy)

leish_yearly_model_agg_quad <- fepois(incidence ~ year_binary*fivekm + log(urban) + log(ag) + sum_precip^2 + mean_temp^2 | cluster + year, 
                                  vcov = "cluster", 
                                  data = leish_df_agg)

leish_biannual_agg_model_dry_quad <- fepois(incidence ~ biannual_binary*fivekm + log(urban) + log(ag) + sum_precip^2 + mean_temp^2 | cluster + biannual_date, vcov = "cluster", data = leish_df_agg_biannual_dry)
leish_biannual_agg_model_rainy_quad <- fepois(incidence ~ biannual_binary*fivekm + log(urban) + log(ag) + sum_precip^2 + mean_temp^2 | cluster + biannual_date, vcov = "cluster", data = leish_df_agg_biannual_rainy)
...
rownames(stable3) <- c("log(Urban)", "log(Ag)", "Sum Precip^2", "Mean Temp^2", "5km x Post-2008 Yearly", "5km x Post-2008 Biannual", "FEs", "Cluster", "Year", "Biannual", "Cor^2", "N", "Clusters")
...
## to determine dengue distribution before paving
# dengue_coverage_yearly <- dengue_df_yearly %>%
...
## population weighting
dengue_yearly_model_pop_weight <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_buffered_no_pm, weights=~population)

## no Puerto Maldonado
dengue_yearly_model_no_PM <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_buffered_no_pm)

## changing the spatial boundary delineating treatment and control groups
dengue_yearly_model_onekm <- fepois(incidence ~ i(year, onekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly)
dengue_yearly_model_tenkm <- fepois(incidence ~ i(year, tenkm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly)

## changing the buffer zone size between treatment and control groups
dengue_df_yearly_no_buffer <- dengue_df_yearly[which(dengue_df_yearly$all_cutoffs %in% c(1,2,3,4,5,6,7,0)),]
dengue_yearly_model_no_buffer <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_no_buffer)
dengue_df_yearly_buffer_bigger <- dengue_df_yearly[which(dengue_df_yearly$all_cutoffs %in% c(1,2,5,6,7,0)),]
dengue_yearly_model_buffer_bigger <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_buffer_bigger)

## confirmed cases only v confirmed and probable cases
dengue_yearly_model_confirmed <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_cp_buffered)

## only include clusters where at least one disease case reported both pre and post treatment
dengue_yearly_model_prepost <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_prepost_buffered)
...
df <- rbind(
  ...
  as.data.frame(cbind(dengue_yearly_model_prepost$coeftable[1:22,],c(rep("Pre post",22)))),
...
df$Model <- factor(as.character(df$Model), levels=c('No PM', 'Pop weight no PM','1km','10km','No buffer', 'Big buffer', 'Confirmed cases', "Pre post", 'Main'))

#consider updating this so the main model is just bold black ouline with no fill in the point estimate
sfig3a <- ggplot(df, aes(group=Model)) +
  ...
  scale_color_manual(name = "Model specification", values=list('No PM'='#DC267F','Pop weight no PM'="#FE6100",'1km'='#FFB000','10km'='#FFE518','No buffer'='#85CE76', 'Big buffer'='#126E29','Confirmed cases'="#648FFF", 'Pre post'="#785EF0", 'Main' = "#2b175e"),
                     labels=list('No PM', 'Pop weight no PM','1km boundary','10km boundary','No buffer (<5km v >5km)', 'Large buffer (<5km v >20km)', 'Conf & prob cases', 'Units reporting pre & post', 'Main model'))+
  scale_fill_manual(name = "Model specification", values=list('No PM'='#DC267F','Pop weight no PM'="#FE6100",'1km'='#FFB000','10km'='#FFE518','No buffer'='#85CE76', 'Big buffer'='#126E29','Confirmed cases'="#648FFF", 'Pre post'="#785EF0", 'Main' = NA),
                    labels=list('No PM', 'Pop weight no PM','1km boundary','10km boundary','No buffer (<5km v >5km)', 'Large buffer (<5km v >20km)', 'Conf & prob cases', 'Units reporting pre & post', 'Main model'))+
...
sfig3b <- ggplot(df_limits, aes(group=Model)) +
  ...
  scale_color_manual(name = "Model specification", values=list('No PM'='#DC267F','Pop weight no PM'="#FE6100",'1km'='#FFB000','10km'='#FFE518','No buffer'='#85CE76', 'Big buffer'='#126E29','Confirmed cases'="#648FFF", 'Pre post'="#785EF0", 'Main' = "#2b175e"),
                     labels=list('No PM', 'Pop weight no PM','1km boundary','10km boundary','No buffer (<5km v >5km)', 'Large buffer (<5km v >20km)', 'Conf & prob cases', 'Units reporting pre & post', 'Main model'))+
  scale_fill_manual(name = "Model specification", values=list('No PM'='#DC267F','Pop weight no PM'="#FE6100",'1km'='#FFB000','10km'='#FFE518','No buffer'='#85CE76', 'Big buffer'='#126E29','Confirmed cases'="#648FFF", 'Pre post'="#785EF0", 'Main' = "#2b175e"),
                    labels=list('No PM', 'Pop weight no PM','1km boundary','10km boundary','No buffer (<5km v >5km)', 'Large buffer (<5km v >20km)', 'Conf & prob cases', 'Units reporting pre & post', 'Main model'))+
  xlab("Year") + ylab("Percent\nchange\ndengue\nincidence") + 
  theme_bw()+
