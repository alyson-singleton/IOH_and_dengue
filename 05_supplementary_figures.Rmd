...
ggsave("SFig6.pdf", plot=sfig6all, path="~/Desktop/doctorate/ch2 mdd highway/supplementary_figures/", width = 6, height = 9, units="in", device = "pdf")
...
## STable 2
#####################
stable2 <- etable(dengue_yearly_model_agg_quad,dengue_biannual_agg_model_dry_quad,dengue_biannual_agg_model_rainy_quad, 
                 leish_yearly_model_agg_quad,leish_biannual_agg_model_dry_quad,leish_biannual_agg_model_rainy_quad, digits = 2)
stable2 <- stable2[c(3:8,9:12,16,15,14),2:7]
stable2[13,] <- c(rep(65,6))
colnames(stable2) <- c('Dengue Yearly', 'Dengue Biannual Dry', 'Dengue Biannual Rainy','Leish Yearly','Leish Biannual Dry', 'Leish Biannual Rainy')
rownames(stable2) <- c("log(Urban)", "log(Ag)", "Mean Precip", "Mean Temp", "5km x Post-2008 Yearly", "5km x Post-2008 Biannual", "FEs", "Cluster", "Year", "Biannual", "Cor^2", "N", "Clusters")
stable2 <- stable2[c(5:6,1:4,7:13),]
stable2
...
## STable 3
#####################
stable3 <- etable(dengue_yearly_model, leish_yearly_model, digits = 2)
stable3 <- stable3[c(3:10,37,11:31,35,34,36),2:3]
stable3[33,] <- c(rep(65,2))
stable3[9,] <- c("----","----")
colnames(stable3) <- c('Dengue Yearly', 'Leish Yearly')
rownames(stable3) <- c("5km x 2000", "5km x 2001", "5km x 2002", "5km x 2003", "5km x 2004", "5km x 2005", "5km x 2006", "5km x 2007", "5km x 2008", "5km x 2009", "5km x 2010", "5km x 2011", "5km x 2012", "5km x 2013", "5km x 2014", "5km x 2015", "5km x 2016", "5km x 2017", "5km x 2018", "5km x 2019", "5km x 2020", "5km x 2021", "5km x 2022", "log(Urban)", "log(Ag)", "Mean Precip", "Mean Temp", "FEs", "Cluster", "Year", "Cor^2", "N", "Clusters")
stable3
...
dengue_biannual_did_popadj <- ggplot(df) +
  ...
  theme_bw()+
  scale_y_continuous(labels = scales::percent, limits=c(-2,10)) +
  theme(plot.title = element_text(size=20),
        plot.title.position = "plot",
        plot.subtitle = element_text(hjust=0.5, size=22),
        axis.title=element_text(size=18),
...
sfig7_legend <- get_legend(dengue_biannual_did_popadj)
...
sfig7all <- grid.arrange(dengue_biannual_did_popadj, leish_biannual_did_popadj, sfig7_legend,
                         ncol = 1, nrow = 3,
                         layout_matrix = rbind(c(1),c(2),c(3)), 
                         heights=c(3,3,1))
...
sfig7all <- as_ggplot(sfig7all) +                                
  draw_plot_label(label = c("A", "B"), size = 14,
                  x = c(0.07, 0.07), y = c(1, 0.58)) 
sfig7all

ggsave("SFig8.pdf", plot=sfig7all, path="~/Desktop/doctorate/ch2 mdd highway/supplementary_figures/", width = 9, height = 10, units="in", device = "pdf")
```