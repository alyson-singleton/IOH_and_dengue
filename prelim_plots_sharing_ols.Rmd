...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure1.pdf", fig1a, width = 8, height = 8, units = c("in"), device="pdf")
```
...
incidence_data_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/yearly_incidence_data.csv")
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/S1.pdf", dengue_yearly_coverage_plot, width = 10, height = 6, units = c("in"), device="pdf")
```
...
```{r}
## data coverage calculation
incidence_data_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/yearly_leish_incidence_data.csv")
incidence_data_yearly$case_yn <- ifelse(incidence_data_yearly$yearly_cases>0, 1, 0)
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/S1_leish.pdf", leish_yearly_coverage_plot, width = 10, height = 6, units = c("in"), device="pdf")

```

# Model specification

In this analysis, I estimate the following regression equation:

$$ y_{it}=\sum\limits_{\substack{\tau\in\{2000,...,2020\},\\\tau\neq2008}} \beta_\tau (DIST)_i \times \mathbb{1}\{t=\tau\}+\gamma_i+\lambda_t+\mathbf{X_{it}\theta}+\epsilon_{it} $$
Our outcome of interest, $y_{it}$, is the dengue incidence in healthcare center $i$ for year $t$. Our treatment variable, $DIST$, is a dummy (binary) variable that equals one for centers near the highway and zero for centers off the highway. The $DIST$ dummy variable interacts with year dummy variables (one for each year of interest and zero otherwise). We omit the last year before the highway was paved (i.e., the last year before treatment) as the baseline year (2008). 
...
incidence_data <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/yearly_incidence_data_pop_adjusted.csv")
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/S2.pdf", fivekm_yearly, width = 10, height = 6, units = c("in"), device="pdf")
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/S3.pdf", all_cutoffs_yearly, width = 10, height = 6, units = c("in"), device="pdf")
...
incidence_data <- read.csv("~/Desktop/doctorate/ch2 mdd highway/processed_case_data/data/yearly_incidence_data.csv")
incidence_data$year <- as.Date(incidence_data$year)
incidence_data$onekm <- as.character(incidence_data$onekm)
incidence_data$fivekm <- as.character(incidence_data$fivekm)
incidence_data$tenkm <- as.character(incidence_data$tenkm)
...
```{r, adjusted pop}
### yearly model
incidence_data_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/yearly_incidence_data_pop_adjusted.csv")
incidence_data_yearly$incidence <- incidence_data_yearly$yearly_cases/incidence_data_yearly$population
incidence_data_yearly$year <- as.factor(incidence_data_yearly$year)
incidence_data_yearly$onekm <- as.numeric(incidence_data_yearly$onekm)
...
incidence_data_yearly <- full_join(incidence_data_yearly, covariates, by=c("cluster"="cluster", "year" = "year"))
incidence_data_yearly_no_pm <- incidence_data_yearly[!(incidence_data_yearly$cluster %in% 1),]
incidence_data_yearly <- incidence_data_yearly[which(incidence_data_yearly$incidence < 0.5),]
test <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') +urban_area+ mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure2.pdf", dengue_yearly_did_popadj, width = 10, height = 6, units = c("in"), device="pdf")
...
test <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly, weights=~population)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, vcov = "cluster", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, data = incidence_data_yearly)
#summary(test)
#class(test)
...
sum(coef(summary(test))[9:20]*pop_1[,3])/pop_total[21,2]
sum((coef(summary(test))[9:20]+test$coeftable[9:20,2])*pop_1[,3])/pop_total[21,2]
sum((coef(summary(test))[9:20]-test$coeftable[9:20,2])*pop_1[,3])/pop_total[21,2]

#percent of dengue cases
sum(coef(summary(test))[9:20]*pop_1[,3])/sum(incidence_data_yearly_cases[c(10:21),2])
...
```{r}
incidence_data <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/biannual_incidence_data_pop_adjusted.csv")
...
```{r}
### biannual model
incidence_data_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/biannual_incidence_data_pop_adjusted.csv")
...
test <- feols(incidence ~ i(biannual_date, fivekm, ref = '2008-04-01') + mean_precip + mean_temp + urban_area | cluster + biannual_date, vcov = "cluster", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, vcov = "cluster", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, data = incidence_data_yearly)
#summary(test)
#class(test)
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure3.pdf", dengue_quarterly_did_popadj, width = 10, height = 6, units = c("in"), device="pdf")
```

```{r, pois biannual, eval=F}
### biannual model
incidence_data_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/biannual_incidence_data_pop_adjusted.csv")
incidence_data_yearly$incidence <- incidence_data_yearly$biannual_cases/incidence_data_yearly$population
incidence_data_yearly$biannual_date <- as.factor(incidence_data_yearly$biannual_date)
...
test <- fepois(incidence ~ i(biannual_date, onekm, ref = '2008-10-01') + mean_precip + mean_temp + urban_area | cluster + biannual_date, vcov = "cluster", data = incidence_data_yearly, weights=~population)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, vcov = "cluster", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, data = incidence_data_yearly)
#summary(test)
#class(test)

#hist(incidence_data_yearly$incidence,breaks=50)
#hist(incidence_data_yearly$incidence[which(incidence_data_yearly$incidence >0)],breaks=50)
#hist(log(incidence_data_yearly$incidence[which(incidence_data_yearly$incidence >0)]),breaks=50)

df <- as.data.frame(test$coeftable)
df <- df[1:41,]
colnames(df) <- c('estimate', 'std_error', 't_value', 'p_value')
df$biannual_date <- as.Date(c(unique(incidence_data_yearly$biannual_date)[c(1:17,19:42)]))
df$rainy <- 0
df$rainy[c(seq(1, 42, 2))] <- 1
df$rainy <- as.character(df$rainy)
df$upper <- df$estimate+1.96*df$std_error
...
dengue_quarterly_did_popadj <- ggplot(df) +
  geom_hline(aes(yintercept=0), colour='red', size=.4) +
  geom_errorbar(aes(x=biannual_date, ymax=upper, ymin=lower), width=0, size=0.5) +
  geom_vline(aes(xintercept=as.Date("2008-10-01")), linetype='dashed', size=0.4) +
  geom_point(aes(x=as.Date("2008-10-01"), y=0), size=3, shape=21, fill='white') +
  geom_point(aes(biannual_date, estimate, fill=rainy), size=3, shape=21) +
  scale_fill_manual(name="Season", values=c('white', '#DC267F'), labels=c('Dry', 'Rainy')) + 
  xlab("Year") + ylab("Difference in\nbiannual dengue\nincidence") + 
  theme_bw()+
...
```{r}
incidence_data <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/yearly_leish_incidence_data_pop_adjusted.csv")
incidence_data$year <- as.Date(incidence_data$year)
incidence_data$onekm <- as.character(incidence_data$onekm)
incidence_data$fivekm <- as.character(incidence_data$fivekm)
incidence_data$tenkm <- as.character(incidence_data$tenkm)
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/S2_leish.pdf", fivekm_yearly_leish, width = 10, height = 6, units = c("in"), device="pdf")
...
```{r}
### yearly model
incidence_data_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/yearly_leish_incidence_data_pop_adjusted.csv")
incidence_data_yearly$incidence <- incidence_data_yearly$yearly_cases/incidence_data_yearly$population
incidence_data_yearly$year <- as.factor(incidence_data_yearly$year)
incidence_data_yearly$onekm <- as.numeric(incidence_data_yearly$onekm)
incidence_data_yearly_no_pm <- incidence_data_yearly[!(incidence_data_yearly$cluster %in% 1),]

#hist(log(incidence_data_yearly$incidence))
...
test <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + mean_precip + mean_temp + urban_area | cluster + year, vcov = "cluster", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, vcov = "cluster", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, data = incidence_data_yearly)
#summary(test)
#class(test)
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure2leish.pdf", leish_yearly_did_popadj, width = 10, height = 6, units = c("in"), device="pdf")
...
```{r}
incidence_data <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/biannual_leish_incidence_data_pop_adjusted.csv")
incidence_data$biannual_date <- as.Date(incidence_data$biannual_date)
incidence_data$onekm <- as.character(incidence_data$onekm)
incidence_data$fivekm <- as.character(incidence_data$fivekm)
incidence_data$tenkm <- as.character(incidence_data$tenkm)
...
```{r}
### biannual model
incidence_data_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/biannual_leish_incidence_data_pop_adjusted.csv")
incidence_data_yearly$incidence <- incidence_data_yearly$biannual_cases/incidence_data_yearly$population
incidence_data_yearly$biannual_date <- as.factor(incidence_data_yearly$biannual_date)
incidence_data_yearly$onekm <- as.numeric(incidence_data_yearly$onekm)
incidence_data_yearly_no_pm <- incidence_data_yearly[!(incidence_data_yearly$cluster %in% 1),]
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure3leish.pdf", leish_quarterly_did_popadj, width = 10, height = 6, units = c("in"), device="pdf")
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure4.pdf", dengue_yearly_did_popadj_spatial_sens, width = 12, height = 6, units = c("in"), device="pdf")
...
# run models
test5km <- fixest::feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test1km <- fixest::feols(incidence ~ i(year, onekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test10km <- fixest::feols(incidence ~ i(year, tenkm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test20km <- fixest::feols(incidence ~ i(year, twentykm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test30km <- fixest::feols(incidence ~ i(year, thirtykm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test40km <- fixest::feols(incidence ~ i(year, fortykm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, vcov = "cluster", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, data = incidence_data_yearly)
#summary(test)
#class(test)
...
df <- rbind(
  ...
  as.data.frame(cbind(test1km$coeftable[1:20,],c(rep("1km",20)))),
  as.data.frame(cbind(test10km$coeftable[1:20,],c(rep("10km",20)))),
  as.data.frame(cbind(test20km$coeftable[1:20,],c(rep("20km",20)))),
  as.data.frame(cbind(test30km$coeftable[1:20,],c(rep("30km",20)))),
  as.data.frame(cbind(test40km$coeftable[1:20,],c(rep("40km",20)))))
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure4v2.pdf", dengue_yearly_did_popadj_spatial_sens_v2, width = 13, height = 6, units = c("in"), device="pdf")

```

## Population weighting sensitivity analysis
```{r, pop weight sensitivity, eval=F}
### yearly model
incidence_data_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/yearly_incidence_data_pop_adjusted.csv")
incidence_data_yearly$incidence <- incidence_data_yearly$yearly_cases/incidence_data_yearly$population
incidence_data_yearly$year <- as.factor(incidence_data_yearly$year)
...
incidence_data_yearly <- incidence_data_yearly[which(incidence_data_yearly$cluster!='16'),]
incidence_data_yearly <- incidence_data_yearly[complete.cases(incidence_data_yearly),]

hist(incidence_data_yearly$incidence, breaks=50)

incidence_data_yearly <- incidence_data_yearly[which(incidence_data_yearly$incidence < 1),]
hist(incidence_data_yearly$incidence, breaks=50)

stor<-log(incidence_data_yearly$incidence)
stor[which(stor==-Inf)] <- 0
#hist(stor)

#build models
test <- fixest::fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "cluster", data = incidence_data_yearly)
test_popweight <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "cluster", data = incidence_data_yearly, weights=~population)
#test_popweight <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly, weights = ~population)
...
#plot(predict(test, incidence_data_yearly), residuals(test))
#plot(predict(test_popweight, incidence_data_yearly), residuals(test_popweight))

df <- rbind(
  as.data.frame(cbind(test$coeftable[1:19,],c(rep("No weighting",19)))),
  as.data.frame(cbind(test_popweight$coeftable[1:19,],c(rep("Pop weighting",19)))))
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/S4.pdf", dengue_yearly_did_popweighting, width = 10, height = 6, units = c("in"), device="pdf")
...
incidence_data_yearly_no_pm <- incidence_data_yearly[!(incidence_data_yearly$cluster %in% 1),]
incidence_data_yearly <- incidence_data_yearly[which(incidence_data_yearly$incidence < 0.5),]
incidence_data_yearly_no_pm <- incidence_data_yearly_no_pm[which(incidence_data_yearly_no_pm$incidence < 0.5),]

#build models
test_wpm <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "cluster", data = incidence_data_yearly)
test_wopm <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "cluster", data = incidence_data_yearly_no_pm)
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure5.pdf", dengue_yearly_did_popadj_pm_sens, width = 10, height = 6, units = c("in"), device="pdf")

```

## Base year sensitivity analysis
```{r, base year sensitivity}
test_2008 <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test_2007 <- feols(incidence ~ i(year, onekm, ref = '2007-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test_2006 <- feols(incidence ~ i(year, tenkm, ref = '2006-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, vcov = "cluster", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, data = incidence_data_yearly)
#summary(test)
#class(test)
...
df <- rbind(
  as.data.frame(cbind(test_2008$coeftable[1:20,],c(rep("2008",20)))),
  as.data.frame(cbind(test_2007$coeftable[1:20,],c(rep("2007",20)))),
  as.data.frame(cbind(test_2006$coeftable[1:20,],c(rep("2006",20)))))
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure6.pdf", dengue_yearly_did_popadj_base_year_sens, width = 12, height = 6, units = c("in"), device="pdf")

```

```{r, base year w 2009 sensitivity, eval=F}
test_2009 <- feols(incidence ~ i(year, tenkm, ref = '2009-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
...
df <- rbind(
  as.data.frame(cbind(test_2009$coeftable[1:20,],c(rep("2009",20)))),
  as.data.frame(cbind(test_2008$coeftable[1:20,],c(rep("2008",20)))),
  as.data.frame(cbind(test_2007$coeftable[1:20,],c(rep("2007",20)))),
  as.data.frame(cbind(test_2006$coeftable[1:20,],c(rep("2006",20)))))
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure6v2.pdf", dengue_yearly_did_popadj_base_year_2009_sens, width = 12, height = 6, units = c("in"), device="pdf")
...
incidence_data_yearly <- full_join(incidence_data_yearly, covariates, by=c("cluster"="cluster", "year" = "year"))
set.seed(100)
incidence_data_yearly_permuted  <- transform(incidence_data_yearly, fivekm = sample(fivekm))   
incidence_data_yearly <- incidence_data_yearly[which(incidence_data_yearly$incidence < 0.5),]
incidence_data_yearly_permuted <- incidence_data_yearly_permuted[which(incidence_data_yearly_permuted$incidence < 0.5),]
...
#build models
test_original <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test_permuted <- feols(incidence ~ i(year, fivekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly_permuted)

df <- rbind(
  as.data.frame(cbind(test_original$coeftable[1:20,],c(rep("Original",20)))),
  as.data.frame(cbind(test_permuted$coeftable[1:20,],c(rep("Permuted",20)))))
...
df$upper <- df$estimate+1.96*df$std_error
df$lower <- df$estimate-1.96*df$std_error
df$Order <- factor(as.character(df$Order), levels=c('Original','Permuted'))
dengue_yearly_did_popadj_permutation_test <- ggplot(df, aes(group=Order)) +
  ...
  scale_color_manual(values=list('Original'="#DC267F",'Permuted'="#FFB000"))+
  scale_fill_manual(values=list('Original'="#DC267F",'Permuted'="#FFB000"))+
  xlab("Year") + ylab("Difference in\nyearly dengue\nincidence") + 
  theme_bw()+
  #ylim(c(-0.02,0.2))+
  theme(plot.title = element_text(size=20),
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure7.pdf", dengue_yearly_did_popadj_permutation_test, width = 10, height = 6, units = c("in"), device="pdf")
...
incidence_data_yearly_buffered <- incidence_data_yearly
incidence_data_yearly_buffered$fivekm <- ifelse(incidence_data_yearly_buffered$fivekm>0, 2, 0)
incidence_data_yearly_buffered$tenkm <- ifelse(incidence_data_yearly_buffered$tenkm>0, 3, 0)
incidence_data_yearly_buffered$twentykm <- ifelse(incidence_data_yearly_buffered$twentykm>0, 4, 0)
incidence_data_yearly_buffered$thirtykm <- ifelse(incidence_data_yearly_buffered$thirtykm>0, 5, 0)
incidence_data_yearly_buffered$fortykm <- ifelse(incidence_data_yearly_buffered$fortykm>0, 6, 0)
for(i in 1:dim(incidence_data_yearly_buffered)[1]){
  incidence_data_yearly_buffered$all_cutoffs[i] <- ifelse(incidence_data_yearly_buffered$onekm[i]>0, 1, 0)
  incidence_data_yearly_buffered$all_cutoffs[i] <- ifelse(incidence_data_yearly_buffered$fivekm[i]>1&&incidence_data_yearly_buffered$onekm[i]==0, 2, incidence_data_yearly_buffered$all_cutoffs[i])
  incidence_data_yearly_buffered$all_cutoffs[i] <- ifelse(incidence_data_yearly_buffered$tenkm[i]>2&&incidence_data_yearly_buffered$fivekm[i]==0&&
                                                            incidence_data_yearly_buffered$onekm[i]==0, 3, incidence_data_yearly_buffered$all_cutoffs[i])
  incidence_data_yearly_buffered$all_cutoffs[i] <- ifelse(incidence_data_yearly_buffered$twentykm[i]>3&&incidence_data_yearly_buffered$tenkm[i]==0&&
                                                            incidence_data_yearly_buffered$fivekm[i]==0&&incidence_data_yearly_buffered$onekm[i]==0, 4, 
                                                          incidence_data_yearly_buffered$all_cutoffs[i])
  incidence_data_yearly_buffered$all_cutoffs[i] <- ifelse(incidence_data_yearly_buffered$thirtykm[i]>4&&incidence_data_yearly_buffered$twentykm[i]==0&&
                                                            incidence_data_yearly_buffered$tenkm[i]==0&&incidence_data_yearly_buffered$fivekm[i]==0&&
                                                            incidence_data_yearly_buffered$onekm[i]==0, 5, incidence_data_yearly_buffered$all_cutoffs[i])
  incidence_data_yearly_buffered$all_cutoffs[i] <- ifelse(incidence_data_yearly_buffered$fortykm[i]>5&&incidence_data_yearly_buffered$thirtykm[i]==0&&incidence_data_yearly_buffered$twentykm[i]==0&&
                                                            incidence_data_yearly_buffered$tenkm[i]==0&&incidence_data_yearly_buffered$fivekm[i]==0&&
                                                            incidence_data_yearly_buffered$onekm[i]==0, 6, incidence_data_yearly_buffered$all_cutoffs[i])
  #center_lat_long$all_cutoffs[i] <- min(center_lat_long$onekm[i],center_lat_long$fivekm[i],center_lat_long$tenkm[i])
}
incidence_data_yearly_buffered_13 <- incidence_data_yearly_buffered[which(incidence_data_yearly_buffered$all_cutoffs %in% c(1,3,4,5,6,0)),]
incidence_data_yearly_buffered_14 <- incidence_data_yearly_buffered[which(incidence_data_yearly_buffered$all_cutoffs %in% c(1,4,5,6,0)),]
incidence_data_yearly_buffered_15 <- incidence_data_yearly_buffered[which(incidence_data_yearly_buffered$all_cutoffs %in% c(1,5,6,0)),]
incidence_data_yearly_buffered_16 <- incidence_data_yearly_buffered[which(incidence_data_yearly_buffered$all_cutoffs %in% c(1,6,0)),]
incidence_data_yearly_buffered_10 <- incidence_data_yearly_buffered[which(incidence_data_yearly_buffered$all_cutoffs %in% c(1,0)),]


# run models
test1km <- feols(incidence ~ i(year, onekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly)
test1_5_buffer <- feols(incidence ~ i(year, onekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly_buffered_13)
test1_10_buffer <- feols(incidence ~ i(year, onekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly_buffered_14)
test1_20_buffer <- feols(incidence ~ i(year, onekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly_buffered_15)
test1_30_buffer <- feols(incidence ~ i(year, onekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly_buffered_16)
test1_40_buffer <- feols(incidence ~ i(year, onekm, ref = '2008-01-01') + urban_area + mean_precip + mean_temp | cluster + year, vcov = "twoway", data = incidence_data_yearly_buffered_10)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, vcov = "cluster", data = incidence_data_yearly)
#test <- feols(incidence ~ i(year, tenkm, ref = '2008-01-01') | cluster + year, data = incidence_data_yearly)
#summary(test)
#class(test)
...
df <- rbind(
  ...
  as.data.frame(cbind(test1_20_buffer$coeftable[1:20,],c(rep("20km_buffered",20)))),
  as.data.frame(cbind(test1_40_buffer$coeftable[1:20,],c(rep("40km_buffered",20)))))
...
ggsave("~/Desktop/doctorate/ch2 mdd highway/presentation_figures_ols/Figure8.pdf", dengue_yearly_did_popadj_buffer_sens, width = 12, height = 6, units = c("in"), device="pdf")
