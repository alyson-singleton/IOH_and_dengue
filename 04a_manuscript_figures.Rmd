...
dengue_yearly_did_popadj <- ggplot(df) +
  ...
  theme_bw()+
  scale_y_continuous(labels = scales::percent, limits=c(-1.2,8)) +
...
dengue_biannual_agg_model_rainy <- fepois(incidence ~ biannual_binary*fivekm + log(urban) + log(ag) + mean_precip + mean_temp | cluster + biannual_date, vcov = "cluster", data = dengue_df_agg_biannual_rainy, weights=~population)
...
fig2all <- grid.arrange(dengue_yearly_did_popadj, dengue_biannual_did_agg,                     
                        ncol = 2, nrow = 1,
...
df <- as.data.frame(rbind(leish_biannual_agg_model_rainy$coeftable[5,],
...
# dengue_df_yearly_1km <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(1,2,3,4,5,6,7,0)),]
# dengue_df_yearly_5km <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(2,3,4,5,6,7,0)),]
# dengue_df_yearly_10km <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(3,4,5,6,7,0)),]
# dengue_df_yearly_15km <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(4,5,6,7,0)),]
# dengue_df_yearly_20km <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(5,6,7,0)),]
# dengue_df_yearly_30km <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(6,7,0)),]
# dengue_df_yearly_40km <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(7,0)),]
...
dengue_yearly_model_30km <- fepois(incidence ~ year_binary*thirtykm + log(urban) + log(ag) + mean_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_30km)
...
df <- as.data.frame(rbind(dengue_yearly_model_1km$coeftable[5,],
                          ...
                          #dengue_yearly_model_30km$coeftable[5,]))
df$Cutoff <- c('1km','5km','10km', '15km','20km')#,'30km')
...
dengue_yearly_did_het_dist <- ggplot(df, aes(group=Cutoff)) +
  ...
  xlab("") + ylab("Percent\nchange\ndengue\nincidence") + 
  theme_bw()+
  scale_y_continuous(labels = scales::percent, limits=c(-1,9.8)) +
  theme(plot.title = element_text(size=20),
        plot.title.position = "plot",
        plot.subtitle = element_text(hjust=0.5, size=22),
        axis.title=element_text(size=14),
...
fig4b <- ggplot() +
  ...
  geom_sf(data = center_lat_long, aes(geometry = geometry, fill=all_cutoffs, color=all_cutoffs), pch=21, size = 3, stroke=0.5) + #colour='#25CED1',
  scale_fill_manual(name= "Legend", values=list('1'="#DC267F",'2'='#FF7900','3'='#FFB000',
                                                '4'='#648FFF','5'='#785EF0','0'='darkgrey'), 
                    labels=c("1km", "5km", "10km", "15km", "20km", "Control")) +
  scale_color_manual(name= "Legend", values=list('1'="#DC267F",'2'='#FF7900','3'='#FFB000',
                                                 '4'='#648FFF','5'='#785EF0','0'='darkgrey'), 
                     labels=c("1km", "5km", "10km", "15km", "20km", "Control")) +
  theme_minimal() +
  no_axis +
...
# use same control group entire time
dengue_df_yearly_1km <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(1,6,7,0)),]
dengue_df_yearly_1km_0s <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(6,7,0)),]
dengue_df_yearly_1km_1s <- dengue_df_agg[which(dengue_df_agg$all_cutoffs %in% c(1)),]

# sample
dengue_df_yearly_1km_sample0s <- sample(dengue_df_yearly_1km_0s$cluster,length(unique(dengue_df_yearly_1km_0s$cluster)),replace=T)
dengue_df_yearly_1km_sample1s <- sample(dengue_df_yearly_1km_1s$cluster,length(unique(dengue_df_yearly_1km_1s$cluster)),replace=T)
ids_to_pull <- c(dengue_df_yearly_1km_sample0s,dengue_df_yearly_1km_sample1s)

# build new dataset
dengue_df_yearly_1km_new <- c()
for(i in ids_to_pull) {
  og_rows_associated_w_id_i <- dengue_df_yearly_1km[which(dengue_df_yearly_1km$cluster == i),]
  dengue_df_yearly_1km_new <- rbind(dengue_df_yearly_1km_new, og_rows_associated_w_id_i)
}

#run model
dengue_yearly_model_1km <- fepois(incidence ~ year_binary*onekm + log(urban) + log(ag) + mean_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_1km_new)
