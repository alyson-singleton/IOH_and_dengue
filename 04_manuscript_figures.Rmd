...
#####################
### yearly model
dengue_yearly_model <- feglm(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + sum_precip + mean_temp | cluster + year, family = "poisson", vcov = "cluster", data = dengue_df_yearly_buffered)
...
fig2a <- ggplot(df) +
  ...
  theme_bw()+
  scale_y_continuous(labels = scales::percent, limits=c(-1.2,17)) +
