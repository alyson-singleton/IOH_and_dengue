...
regional_incidence_all <- dengue_yearly$connected_buffered %>%
  ...
  summarise(
    ...
    incidence_all = yearly_cases_all / pop_sum_all)
...
regional_incidence_yearly <- dengue_yearly$connected_buffered %>%
  ...
  summarise(
    yearly_cases = sum(yearly_cases),
    pop_sum = sum(population),
    incidence_avg = yearly_cases / pop_sum,
    .groups = "drop") %>%
...
treated_post <- treated_post %>%
  mutate(expected_cases = treated_pre_avg_incidence * population)
...
df$estimate <- as.numeric(df$estimate)*1000
...
df$upper <- df$estimate+1.96*df$std_error_boot*1000
df$lower <- df$estimate-1.96*df$std_error_boot*1000
