...
```{r, simple for gee workshop, eval=F}
...
fig1a <- ggplot() +
  geom_sf(data=districts, fill='#ffffff', color='#626262', size=.15, show.legend = FALSE) +
  geom_sf(data = highway_mdd, aes(geometry = geometry), colour='black', linewidth=0.7) +
  geom_sf(data = center_lat_long, aes(geometry = geometry), fill='#E04490', color='#E04490', pch=21, size = 2, alpha=0.6) + #colour='#25CED1',
  # scale_fill_manual(name= "Legend", values=c("0" = "#648FFF", "2" = "#E04490", "3" = "#ffffff"), #41b6c4 #fccccc
  #                   labels=c("Control (>10km)", "Treated (<5km)", "Buffer (removed)")) +
  theme_minimal() +
  no_axis +
...
ggsave("~/Desktop/Fig2BQW.pdf", fig1a, width = 6, height = 6, units = c("in"), device="pdf")
```
...
### yearly data
malaria_df_yearly <- read.csv("~/Desktop/doctorate/ch2 mdd highway/data/processed_case_data/malaria_yearly_full_dataset.csv")
malaria_df_yearly$year <- as.factor(malaria_df_yearly$year)

# cases + 1 so 0's dont go to infinity when logged
malaria_df_yearly$incidence <- (malaria_df_yearly$yearly_cases+1)/malaria_df_yearly$population 

# land use vars + 0.001 so 0's dont go to infinity when logged
malaria_df_yearly$urban <- malaria_df_yearly$urban + 0.001
malaria_df_yearly$ag <- malaria_df_yearly$ag + 0.001

# double check complete cases
malaria_df_yearly <- malaria_df_yearly[complete.cases(malaria_df_yearly),]

# remove extreme outliers (all most likely due to inaccurate pop data)
malaria_df_yearly <- malaria_df_yearly[which(malaria_df_yearly$incidence < 0.5),]

# check how many are always zero
malaria_df_yearly_zeroes <- malaria_df_yearly %>% 
  group_by(cluster) %>% 
  summarize(total_cases = sum(yearly_cases))
clusters_w_at_least_one_case <- malaria_df_yearly$cluster[which(malaria_df_yearly_zeroes$total_cases>0)]
malaria_df_yearly_at_least_one_case <- malaria_df_yearly[which(malaria_df_yearly$cluster %in% clusters_w_at_least_one_case),]

# remove PM
malaria_df_yearly_no_pm <- malaria_df_yearly[!(malaria_df_yearly$cluster %in% 1),]

# build final, buffered dataset where below 5km is 1, between 5km and 10km are thrown out (buffer zone), and above 10km is 0
malaria_df_yearly_buffered <- malaria_df_yearly[which(malaria_df_yearly$all_cutoffs %in% c(1,2,4,5,6,0)),]
malaria_df_yearly_buffered_no_pm <- malaria_df_yearly_buffered[!(malaria_df_yearly_buffered$cluster %in% 1),]
```
...
#fivekm
fivekm_plotting <- malaria_df_yearly_buffered %>%
  group_by(fivekm, year) %>%
  summarize(new_incidence = sum(yearly_cases)/sum(population))
...
```{r, main specification malaria}
### yearly model
dengue_yearly_model <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + mean_precip + mean_temp | cluster + year, vcov = "cluster", data = malaria_df_yearly_buffered)
#summary(dengue_yearly_model)
#print(broom::tidy(dengue_yearly_model, exponentiate = TRUE, conf.int = TRUE), n=25)
...
dengue_yearly_did_popadj <- ggplot(df) +
  ...
  xlab("Year") + ylab("Percent\nchange\ndengue\nincidence") + 
  theme_bw()+
  scale_y_continuous(labels = scales::percent, limits=c(-3,5.5)) +
  theme(plot.title = element_text(size=20),
        plot.title.position = "plot",
        plot.subtitle = element_text(hjust=0.5, size=22),
        axis.title=element_text(size=12),
...
```{r, spatial boundary sensitivity}
...
```{r, spatial boundary sensitivity v2, eval=F}
