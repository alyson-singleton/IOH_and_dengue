...
pacman::p_load(tidyverse, dplyr, kableExtra, knitr, sf, raster, terra, spData, tmap, leaflet, ggplot2, spThin, mapdata, rnaturalearth, rnaturalearthdata, brazilmaps, devtools, brazilmaps, mapview, grid, gridExtra, RColorBrewer, raster, plm, fixest, cobalt,broom)
theme_set(theme_bw())
...
dengue_df_yearly_buffered <- dengue_df_yearly[which(dengue_df_yearly$all_cutoffs %in% c(1,2,4,5,6,0)),]
...
# build final, buffered dataset where below 5km is 1, between 5km and 10km are thrown out (buffer zone), and above 10km is 0
leish_df_yearly_buffered <- leish_df_yearly[which(leish_df_yearly$all_cutoffs %in% c(1,2,4,5,6,0)),]
...
all_cutoffs_yearly <- ggplot(incidence_data_yearly_buffered_plotting) +
  ...
  #ggtitle("B. straminea") +
  xlab("Year") + ylab("Yearly\nincidence") + 
  scale_color_manual(name = "Treatment", values=c('#DC267F', 'darkorange', '#FFB000','#648FFF',"#785EF0","darkgreen"))+#, labels=c('<1km', '[1km, 10km)', '[10km, 30km)', '>30km')) +
  #scale_color_manual(name="Distance",values=list('2'="#DC267F",'3'="#FFB000",'4'='#648FFF', '0'="#785EF0"))+
  theme_bw()+
  theme(plot.title = element_text(hjust=0.5, size=26, face="italic"),
        plot.subtitle = element_text(hjust=0.5, size=22),
...
#build models
dengue_yearly_model_unweighted <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + mean_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_buffered_popgt1000, weights=~population)
dengue_yearly_model_weighted <- fepois(incidence ~ i(year, fivekm, ref = '2008-01-01') + log(urban) + log(ag) + mean_precip + mean_temp | cluster + year, vcov = "cluster", data = dengue_df_yearly_buffered_poplt1000, weights=~population)

df <- rbind(
  as.data.frame(cbind(dengue_yearly_model_unweighted$coeftable[1:22,],c(rep("Pop >1000",22)))),
  as.data.frame(cbind(dengue_yearly_model_weighted$coeftable[1:22,],c(rep("Pop <1000",22)))))
...
## Buffer zone sensitivity analysis (Figure 4?)
```{r, buffer zone sensitivity 5km}
# build easily accessible buffer datasets for each buffer option
dengue_df_yearly$fivekm <- as.numeric(dengue_df_yearly$fivekm)
dengue_df_yearly_buffered_23 <- dengue_df_yearly[which(dengue_df_yearly$all_cutoffs %in% c(1,2,3,4,5,6,0)),]
dengue_df_yearly_buffered_24 <- dengue_df_yearly[which(dengue_df_yearly$all_cutoffs %in% c(1,2,4,5,6,0)),]
dengue_df_yearly_buffered_25 <- dengue_df_yearly[which(dengue_df_yearly$all_cutoffs %in% c(1,2,5,6,0)),]
dengue_df_yearly_buffered_26 <- dengue_df_yearly[which(dengue_df_yearly$all_cutoffs %in% c(1,2,6,0)),]
dengue_df_yearly_buffered_20 <- dengue_df_yearly[which(dengue_df_yearly$all_cutoffs %in% c(1,2,0)),]
