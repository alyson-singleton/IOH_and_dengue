...
sum((coef(summary(test))[9:20]+test$coeftable[9:20,2])*pop_1[,3])/pop_1[12,3]
sum((coef(summary(test))[9:20]-test$coeftable[9:20,2])*pop_1[,3])/pop_1[12,3]
```
...
df <- rbind(
  as.data.frame(cbind(test1km$coeftable[1:20,],c(rep("1km",20)))),
  as.data.frame(cbind(test1_5_buffer$coeftable[1:20,],c(rep("5km_buffered",20)))),
  ...
  as.data.frame(cbind(test1_30_buffer$coeftable[1:20,],c(rep("30km_buffered",20)))),
...
df$upper <- df$estimate+1.96*df$std_error
df$lower <- df$estimate-1.96*df$std_error
df$Cutoff <- factor(as.character(df$Cutoff), levels=c('1km','5km_buffered','10km_buffered','20km_buffered','30km_buffered','40km_buffered'))
dengue_yearly_did_popadj_buffer_sens <- ggplot(df, aes(group=Cutoff)) +
  ...
  geom_point(aes(x=as.Date("2008-01-01"), y=0), size=3, shape=21, fill='white') +
  geom_point(aes(year, estimate, fill=Cutoff), color='black',position = position_dodge(width=250), size=3, shape=21) +
  scale_color_manual(name="Treament, control", values=list('1km'="#648FFF",'5km_buffered'="#785EF0",'10km_buffered'='#DC267F',
                                                   '20km_buffered'='#FE6100','30km_buffered'='#FFB000','40km_buffered'='#FFE518'), 
                     labels=list("1km"="<1km, >1km", "5km_buffered"="<1km, >5km", "10km_buffered"="<1km, >10km",
                                 "20km_buffered"="<1km, >20km", "30km_buffered"="<1km, >30km", "40km_buffered"="<1km, >40km"))+
  scale_fill_manual(name="Treament, control", values=list('1km'="#648FFF",'5km_buffered'="#785EF0",'10km_buffered'='#DC267F',
                                                  '20km_buffered'='#FE6100','30km_buffered'='#FFB000','40km_buffered'='#FFE518'), 
                    labels=list("1km"="<1km, >1km", "5km_buffered"="<1km, >5km", "10km_buffered"="<1km, >10km",
                                 "20km_buffered"="<1km, >20km", "30km_buffered"="<1km, >30km", "40km_buffered"="<1km, >40km"))+
  xlab("Year") + ylab("Difference in\nyearly dengue\nincidence") + 
