...
```{r, STable4*}
#intercept terms from main models
```
...
sfig3
ggsave("SFig3.pdf", plot=sfig3, "~/Desktop/doctorate/ch2 mdd highway/supplementary_figures/", width = 12, height = 7, units = c("in"), device="pdf")
```
...
sfig4a <- ggplot() +
  geom_histogram(aes(permuted_effects_stor_full), binwidth = 0.1, fill="grey") +
...
sfig4b <- ggplot() +
  geom_histogram(aes(permuted_effects_stor_block), binwidth = 0.1, fill="grey") +
  geom_vline(xintercept=dengue_yearly_model_agg_effect_est, color='red') +
  xlab("Bootstrapped effect estimate") + ylab("") + 
  theme_bw()+
  theme(plot.title = element_text(hjust=0.5, size=26, face="italic"),
...
sfig4all <- as_ggplot(sfig4all) +                                
  draw_plot_label(label = c("A", "B","C"), size = 13,
                  x = c(0.036, 0.369, 0.714), y = c(0.99, 0.99, 0.99)) 
sfig4all
...
```{r, scratch (orginal clustering fig), eval=F}
#####################
